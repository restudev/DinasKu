...
class CheckpointController extends Controller
{
    ...
    public function generate(
        ...
        */
        foreach ($checkpointData['checkpoints'] as $item) {
            ...
            $checkpoint->qr_image = $qr['image'];      // path di Supabase
            $checkpoint->qr_url   = $qr['image_url'];  // URL publik Supabase
    ...
    public function detail($id)
    {
        ...
        // qr_url sudah tersimpan sebagai URL Supabase penuh di DB
        // Jika kolom masih kosong (data lama), fallback ke path lokal
        if (!$checkpoint->qr_url && $checkpoint->qr_image) {
            $checkpoint->qr_url = asset('storage/' . $checkpoint->qr_image);
        }
    ...
    public function generateQr(
        ...
        $checkpoint->update([
            'qr_token'  => $qr['token'],
            'qr_image'  => $qr['image'],      // path di Supabase
            'qr_url'    => $qr['image_url'],  // URL publik Supabase
        ]);
